...
@Component({
  selector: 'app-baja-unica',
  templateUrl: './baja-unica.component.html',
  styleUrls: ['./baja-unica.component.css']
})
export class BajaUnicaComponent implements OnInit {
  ...
  guardar() {
    if (this.hash.length > 35) {
      if (this.checked) {
        this.adminService.deleteNetworkUnClaim(this.selectedOrg, this.name, this.nameSerie, this.hash, 0, [], '');
      } else {
        this.adminService.deleteNetwork(this.selectedOrg, this.name, this.hash);
      }
    } else {
